...
NUM_REPLICAS=${2:-$1}
LR=${3:-0.1}
RESUME=/data/users/sgross/resnet50-8x8x32-epoch20.pt
if [ -z $NUM_LOCAL ]; then
...
do
  ...
  python3 -u main.py -a resnet50 --num-replicas $NUM_REPLICAS --resume $RESUME --epochs 135 -b 32 --lr $LR --checkpoint-dir $CHECKPOINT_DIR $IMAGENET_DIR &>replica$i.log &
done
wait
